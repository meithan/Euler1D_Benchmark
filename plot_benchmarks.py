...
ref_idx = [x[0] for x in results].index("C/C++")
...
colors = ["C0", "C3", "C1", "C2", "C4", "C5", "C6", "C7", "C8", "C9"]
...
fig, axs = plt.subplots(1, 2, figsize=(10,5), gridspec_kw={'width_ratios': [len1, len2]})
...
for subnum, _results in enumerate([results1, results2]):
  ...
  labels, ratios, means, minvs, maxvs, minrs, maxrs = zip(*_results)
...
labels, ratios, means, minvs, maxvs, minrs, maxrs = zip(*results)
labels = [x.replace("+numpy", "\n+numpy") for x in labels]
labels = [x.replace("Julia+", "Julia\n+") for x in labels]
yerrs = np.abs(np.array([minrs, maxrs]) - ratios)
